...
for i in {0..5}; do
    ...
    # echo $NOME
    PRECO=$(seq 0 1.1 100.9 | shuf | head -n1 | tr , .)
    # echo $PRECO
    curl -X POST "http://localhost:4000/api/Books" -H "accept: text/plain" -H "Content-Type: application/json-patch+json" -d "{\"_id\":\"\",\"BookName\":\"Clean Code\",\"Price\":$PRECO,\"Category\":\"Computers\",\"Author\": $NOME}"
done
